...
struct AboutView: View {
    ...
    // MARK: - Preferences (music)

    private var preferencesCard: some View {
        SettingsCard(title: "about.section.preferences") {
            toggleRow(
                icon: music.isEnabled ? "music.note" : "speaker.slash.fill",
                tintColors: music.isEnabled
                    ? [Color(red: 0.85, green: 0.45, blue: 0.95),
                       Color(red: 0.55, green: 0.35, blue: 0.95)]
                    : [.white.opacity(0.18), .white.opacity(0.08)],
                title: "about.sound.title",
                subtitle: "about.sound.subtitle",
                isOn: Binding(
                    get: { music.isEnabled },
                    set: {
                        music.isEnabled = $0
                        Analytics.track(.musicToggled, ["enabled": $0])
                    }
                )
            )
        }
